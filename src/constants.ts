...
export const FIX_LIBRARY = {
  CRITICAL: {
    ...
    action: "1) openclaw cron list --all — find this job\n2) openclaw cron edit [JOB_ID] --every 30m (or any interval >= 30min)\n3) Or if it is a simple check: openclaw cron disable [JOB_ID], re-create as plain cron\n4) Re-import ~/.openclaw/cron/jobs.json to verify the change",
  ...
  LLM_AGENT_CRON: {
    ...
    action: "1) openclaw cron list --all — find this job\n2) openclaw cron disable [JOB_ID] to stop the waste immediately\n3) Re-import ~/.openclaw/cron/jobs.json to verify the change",
  ...
  PREMIUM_MODEL_WASTE: {
    ...
    action: "1) openclaw cron list --all — find this job\n2) openclaw cron edit [JOB_ID] --model mini-max/m2.7\n3) openclaw cron run [JOB_ID] to verify it still works\n4) Monitor the next 3 runs to confirm output quality",
