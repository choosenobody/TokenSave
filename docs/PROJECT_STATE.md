...
**Last updated**: 2026-05-07T12:25:00Z
**Source**: GitHub `origin/main` at commit `da48c3d` (PR #115 fix(I15-A): OpenClaw CLI guidance — cron command family + local data paths; merge commit `da48c3d`)
...
| Main branch SHA | `da48c3d` (PR #115 fix(I15-A): OpenClaw CLI guidance — cron command family + local data paths; merge commit `da48c3d`) |
| Package manager | npm |
...
- **I15-B (Active vs Historical Waste Distinction) — next implementation slice.** Distinguish active enabled recurring waste from disabled/historical waste so TokenSave does not overstate urgency for jobs that are already disabled or exist only as historical/unmapped run records. Active enabled jobs → main Top Waste / Fix Cards. Disabled/historical → separate "Historical / disabled review signals" section, visually muted. Synthetic/unmapped jobs → labeled "Unmapped historical run". Missing enabled field → treated as active/unknown (do not hide). No backend/network/telemetry. Tests required.
...
- `npm test` passes 316 tests (PR #115: 55 tests in tests/i15a-openclaw-cli-guidance.test.ts; total suite 9 files, 316 tests).
