...
| src/advisory.ts | 653 lines, `@ts-nocheck` pure helper module for the strictly lower-priority review-signal layer (I23). Exports `AdvisoryId` / `AdvisorySignal` types, `pickPresentTokenValue` + `hasZeroTokenValue` helpers (avoid `Number(null) === 0` false positive on missing token fields), and detectors `detectA1ZeroTokenFastFailure`, `detectA2PremiumNoReply`, `detectA3SharedFailureSignature` plus `failureSignature` normalizer and `rankAdvisorySignals` ranking helper. Every signal emits "Review signal (not confirmed waste)" disclaimer; approximate cost exposure is `{lowUsd, highUsd}` for A1 only and `null` for A2/A3. NOT imported by `src/main.ts` (opt-in) |
...
**Recommended Next Step**: Wire the advisory layer into the existing analyzeDataset / renderReport pipeline in a follow-up PR (decide separately whether A1/A2/A3 should surface as a third section between fix cards and historical review, or alongside historical review as another muted "review signal" section). Until then the module is dormant but tested.
