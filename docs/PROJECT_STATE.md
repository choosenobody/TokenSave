...
**Last updated**: 2026-06-04T15:49:38Z
**Source**: GitHub `origin/main` at commit `7c198c2` (PR #155 fix(advisory): cover real Hermes nested token schema in A1; gate A1 on runCount>=2; merge commit `7c198c2`)
...
| Main branch SHA | `7c198c2` (PR #155 fix(advisory): cover real Hermes nested token schema in A1; gate A1 on runCount>=2; merge commit `7c198c2`) |
| Package manager | npm |
...
| src/advisory.ts | 773 lines, `@ts-nocheck` pure helper module for the strictly lower-priority review-signal layer (I23 + I24 + I25). Exports `AdvisoryId` / `AdvisorySignal` types, `pickPresentTokenValue` + `hasZeroTokenValue` + `parseTokenNumber` helpers (avoid `Number(null) === 0` and `Number('') / Number(false) / Number([]) → 0` false positives on missing or non-numeric token fields at both top-level and nested `usage.total_tokens` / `usage.tokens` / `metrics.tokens` paths), and detectors `detectA1ZeroTokenFastFailure`, `detectA2PremiumNoReply`, `detectA3SharedFailureSignature` plus `failureSignature` normalizer and `rankAdvisorySignals` ranking helper. I25: `parseTokenNumber` is a strict whitelist — accepts finite numbers only, rejects null/undefined/blank/whitespace strings/booleans/objects/arrays/NaN/Infinity/all strings (numeric strings included in strict mode); both top-level and nested paths delegate to it. I24: `pickPresentTokenValue` reads Hermes-style nested `usage.total_tokens` / `usage.tokens` / `metrics.tokens` after top-level aliases, and A1 requires `MIN_A1_RUN_COUNT = 2` qualifying runs per job (single-run patterns treated as dispatcher jitter, not confirmed pattern). Every signal emits "Review signal (not confirmed waste)" disclaimer; approximate cost exposure is `{lowUsd, highUsd}` for A1 only and `null` for A2/A3. NOT imported by `src/main.ts` (opt-in) |
...
| src/main.ts | ~1200 lines, `@ts-nocheck`, application logic (ingest/analyzeDataset/finalizeStat/render UI helpers; detectCostRate moved to src/pricing.ts; buildFixCards moved to fixes.ts; finalizeStat attaches evidence to FinalizedJob; all pure helpers extracted to domain/utils/fixes); I4-C1: token/waste-first UI, default sort by tokens, Approx. Cost Exposure wording, cost demoted to secondary signal; I9-A: renderImportSummary panel above summary grid; handleFiles sets dataset.fileCount = files.length for real import path; I9-B: renderImportSummary extended with readiness gap sections (present/missing evidence tags, affected diagnostics, manual next steps); renderFixes restraint logic: fix cards suppressed only when hasJobs===false && hasRuns===false; I10-B1 maps low-level parse failures to clearer import guidance messages; I13-A: renderImportSummary audit-strength framing note (full/partial/limited evidence → actionable implications), renderFixes restrained-state copy clarifies CLI-only/manual guidance; I15-A: buildFixSteps rewritten — one command per job ID to avoid multi-ID command risk; ERROR_WASTE uses `cron show` + `cron runs --id` per job; buildFixSteps now splits both comma-separated and whitespace-separated ID lists (split(/[,\\s]+/)); importTip updated to reference ~/.openclaw/cron/jobs.json re-import; I15-B: lifecycleStatus assignment per job, activeJobs/historicalJobs split, report.historicalJobs, renderTopWaste secondary 'Historical / disabled review signals' section (muted cards), renderFixes historical fix cards with 'Review signal' badge + grey tags + inspect-only commands; I18-B: ingestZipFile normalizes Windows backslash paths to forward slashes for run*.jsonl detection; I20: fix card HTML restructured with explicit Problem/Evidence/Manual Fix/Verify section labels; stale-snapshot amber warning in renderImportSummary (only fires when `lifecycleStatus === 'historical'` synthetic/unmatched records present, disabled jobs alone do not trigger); OK fix cards suppress misleading "Token waste patterns detected" evidence fallback text; I21: renderFixes active card loop restructured — job-level cards (one job per card), "Start here: inspect this job first" header on first card with `.first-action-card` CSS highlight, "Why This Is First" / "Why Inspect This Job" section per card using `buildWhyFirstText()`, Safe Inspect Command section with `buildInspectSteps()` (read-only cron show + cron runs --id --limit 5 for all categories); historical/disabled review moved to collapsible `<details>` with `<summary>` (not auto-expanded); stale snapshot warning scoped to `hasSyntheticJobs` only; `buildFixSteps` for ERROR_WASTE outputs inspect-first, edit/disable steps post-inspect; Cost by Job pie chart title demoted to "(Secondary)"; `buildFixCards` now returns per-job cards (not category-level), `historicalFixes` simplified to `historicalJobs.filter(job => job.badge !== 'OK')`; reboot/v0-action-first (PR #135): renderReport() restructured — renderFixes called FIRST before full report shell; exactly one "Start here: inspect this job first" card renders first (fixes[0] with `.first-action-card` cyan highlight); rest of active jobs in collapsed `<details class="other-findings">`; full report shell wrapped in `<details id="fullReportDetails">` collapsed by default (open=false); comment corrected to "Keep the full report shell collapsed by default"; other-findings summary label has no hard-coded ▼ (CSS ::before controls arrow state); post-smoke first-card waste display (PR #137): buildCardHtml computes estimateDailyWasteTokens/estimateWastePerRun for first card only (isFirst && priorityBasis guard), calls buildPriorityBasisText() for formatted output (Tier 1: ~N tokens/day, Tier 2: ~N tokens/run + schedule unavailable note, Tier 3: fallback), Estimated Recurring Waste section inserted between "Why This Is First" and Evidence on first card only; Evidence block preserved unconditionally for all cards; first action remains read-only inspect commands (no edit/disable/enable); no parser/domain/rules/pricing/fixes changes; no backend/network/telemetry/runtime write path |
...
| tests/advisory-signals.test.ts | 973 lines, 97 tests — I23 + I24 + I25 advisory review-signal contract tests: A1 zero-token fast failure (with `pickPresentTokenValue` missing-token false-positive regression); A2 premium model + repeated NO_REPLY (with null-exposure regression); A3 cross-job shared failure signature (allowlist-reject / dispatcher-reject / normalized-error / zero-token-fast-failure); safety copy (first action contains no disable/edit/restart/switch-model); ranking invariant (confirmed D-rule IDs demote advisory signals to reviewSignals); rankAdvisorySignals does not mutate input; I24: nested Hermes-style token-path coverage (`usage.total_tokens` / `usage.tokens` / `metrics.tokens`); A1 `runCount >= 2` noise-floor gate; failureSignature now classifies all three nested paths as `zero-token-fast-failure`; I25: strict `parseTokenNumber` whitelist tests (finite numbers only; rejects null / undefined / blank and whitespace strings / booleans / objects / arrays / NaN / Infinity / numeric strings in strict mode); both top-level and nested paths delegated to parseTokenNumber (closing the `Number('') / Number(false) / Number([]) → 0` false-zero trap); 9 explicit trap-input tests + positive controls + adversarial harness; A1 safety-copy test uses same jobId with `signals.length > 0` guard so it cannot pass vacuously; total suite 651 tests / 16 files |
| tests/i22-agent-prompt-actionable.test.ts | ~400 lines, 48 tests — I22 agent diagnosis prompt primary action source-level UI contract tests: verifies Problem Summary / What To Do First / Agent Prompt visible flow (first action card default state); verifies old "Start here: inspect this job first" language absent from buildCardHtml; verifies first-card verification guidance collapsed under `<details class="advanced-details">`; verifies no `openclaw cron run` in first-card verification; verifies agent prompt classification output, prompt safety requirements, and generated guidance structure; total suite 554 tests / 15 files (at PR #139 merge — historical; current main @ 7c198c2 is 651 tests / 16 files, see Current Local Validation Reality below) |
...
**Recommended Next Step**: Wire the advisory layer into the existing analyzeDataset / renderReport pipeline in a follow-up PR (decide separately whether A1/A2/A3 should surface as a third section between fix cards and historical review, or alongside historical review as another muted "review signal" section). Two natural sub-decisions are now open: (a) source-aware framing — A1/A2 are best framed as "openclaw-shape signals" while A3 is the only signal-bearing detector on real Hermes-style nested data, so any UI copy should distinguish the two; (b) strict-mode vs numeric-string-accepting parser — current `parseTokenNumber` is strict, but if a future export source needs numeric-string token support, it should be added explicitly with non-empty trimmed numeric-string tests while keeping booleans/blanks/arrays rejected. Until then the module is dormant but tested.
...
- `npm run build` passes (PR #155 baseline, 2026-06-04T15:49:38Z): `dist/index.html` 13.15 kB, `dist/app/index.html` 35.30 kB, `dist/assets/app-*.js` 58.88 kB (gzip 18.16 kB). The `30.00 kB HTML` baseline referenced in older snapshots (PR #139 era) predates the `reboot/v0-action-first (PR #135)` HTML-shell split into `dist/index.html` + `dist/app/index.html` and is no longer accurate; the JS bundle size 58.88 kB / 18.16 kB gzip is unchanged from PR #139 because PR #155 only added `src/advisory.ts` which is not yet imported by `src/main.ts` (advisory layer remains dormant / opt-in).
- `npm test` passes 651 tests (16 test files), including 97 in `tests/advisory-signals.test.ts` (37 I23 + 21 I24 + 39 I25).
