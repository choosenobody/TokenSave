...
**Source**: GitHub `origin/main` at commit `2adf51cd` (PR #152 feat(advisory): add A1/A2/A3 review-signal layer (I23); merge commit `2adf51cd`)
...
| Main branch SHA | `2adf51cd` (PR #152 feat(advisory): add A1/A2/A3 review-signal layer (I23); merge commit `2adf51cd`) |
| Package manager | npm |
...
| src/advisory.ts | ~653 lines, pure advisory/review-signal layer (I23 / PR #152): AdvisorySignal type + A1 zero-token fast-failure detector + A2 premium-model repeated NO_REPLY detector + A3 cross-job shared failure signature detector + rankAdvisorySignals(); explicitly review-only, not confirmed waste; approximateCostExposure is null when unverifiable; A1/A3 require an actual token field explicitly equal to 0 before emitting zero-token-fast-failure; no backend/network/telemetry/runtime write path |
...
| tests/advisory-signals.test.ts | 37 tests for I23 advisory review signals: A1/A2/A3 detectors, A2 null approximateCostExposure contract, missing-token false-positive regressions for A1/A3, safety copy forbidding mutation-first actions, shared dispatcher/profile/gateway inspection guidance, and rankAdvisorySignals confirmed-vs-advisory separation; full suite now 591 tests / 16 files |
...
|----|-------|--------|-------------|
| #152 | feat(advisory): add A1/A2/A3 review-signal layer (I23) | 2026-06-03 | `2adf51cd` |
| #150 | docs(PROJECT_STATE): refresh after PR #149 — self-inconsistency fix | 2026-06-03 | `72a14ef` |
...
| I23 (Advisory Review-Signal Layer) | Added pure review-only advisory layer in `src/advisory.ts`: A1 zero-token fast failure, A2 premium repeated NO_REPLY, A3 shared failure signature, plus rankAdvisorySignals. Confirmed D1-D7 remains primary; advisory signals are not confirmed waste and first actions are read-only inspect. 37 advisory tests; total suite 591 tests / 16 files. | #152 | **CLOSED** |
...
**I23 (Advisory Review-Signal Layer, PR #152) — CLOSED.** Added a dormant pure helper module (`src/advisory.ts`) for review-only A1/A2/A3 signals based on agent-first dogfood. The module is not wired into `src/main.ts` yet. A2 unknown exposure returns `approximateCostExposure: null`; A1/A3 zero-token-fast-failure requires an actual token field explicitly equal to 0; missing token evidence does not fire zero-token fast failure. 37 advisory tests; total suite 591 tests / 16 files.
...
- `npm run build` passes (58.88 kB JS / 35.30 kB app HTML, PR #152).
- `npm test` passes 591 tests (16 test files).
