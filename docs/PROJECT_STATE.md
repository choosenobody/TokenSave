...
**Last updated**: 2026-06-05T08:42:57Z
**Source**: GitHub `origin/main` at commit `1b0592a` (PR #157 feat(advisory): add source-aware Agent Cron Audit readiness framing (W-light); old main `7a283ddef8595f8a9ffc3c319e35ecf8d9658429`; implementation head `e02277b1b80b9eea9e57801857c6c33241bf3d9c`; merge commit `1b0592a19bf663a40745dcb0992e16daaf292609`)
...
| Main branch SHA | `1b0592a` (PR #157 feat(advisory): source-aware Agent Cron Audit readiness framing W-light; merge commit `1b0592a`; old main before merge `7a283ddef8595f8a9ffc3c319e35ecf8d9658429`) |
| Package manager | npm |
...
| src/advisory.ts | 858 lines, `@ts-nocheck` pure helper module for the strictly lower-priority review-signal layer (I23 + I24 + I25 + PR #157 W-light). Exports `AdvisoryId` / `AdvisorySignal` types, `SourceShape`, `DetectOptions`, `pickPresentTokenValue` + `hasZeroTokenValue` + `parseTokenNumber` helpers, and detectors `detectA1ZeroTokenFastFailure`, `detectA2PremiumNoReply`, `detectA3SharedFailureSignature` plus `failureSignature` normalizer and `rankAdvisorySignals`. I25: `parseTokenNumber` is a strict whitelist — accepts finite numbers only, rejects null/undefined/blank/whitespace strings/booleans/objects/arrays/NaN/Infinity/all strings; both top-level and nested paths delegate to it. PR #157 W-light: detectors accept optional `sourceShape`; hermes-cron adds shape-specific explanation notes while thresholds, first actions, ranking, disclaimer, and default/openclaw-export/unknown copy remain stable. Every signal emits "Review signal (not confirmed waste)"; approximate cost exposure is `{lowUsd, highUsd}` for A1 only and `null` for A2/A3. NOT imported by `src/main.ts` (opt-in) |
...
| src/main.ts | ~1200 lines, `@ts-nocheck`, application logic (ingest/analyzeDataset/finalizeStat/render UI helpers; detectCostRate moved to src/pricing.ts; buildFixCards moved to fixes.ts; finalizeStat attaches evidence to FinalizedJob; all pure helpers extracted to domain/utils/fixes); I4-C1: token/waste-first UI, default sort by tokens, Approx. Cost Exposure wording, cost demoted to secondary signal; I9-A/I9-B: renderImportSummary panel with audit readiness and readiness gap sections; PR #157 W-light: renderImportSummary also renders "Source-shape interpretation" when `summary.sourceShape` is present, using pure parser copy; advisory remains dormant because `src/main.ts` still does not import `src/advisory.ts`. Other active actionability, historical/disabled review, first-card prompt, collapsed full-report shell, and no-network/no-write constraints remain unchanged. |
...
| src/parser.ts | 445 lines, parseJson / parseJsonl / parseZipEntries + private ZIP helpers + exported detectImportSource(dataset) pure function (source type detection, sourceShape detection, audit confidence, supportedRuleHint, evidence hints) + private hasFiniteTokenField helper (zero-token alias detection); I9-B: exported buildReadinessGaps(summary) pure function — maps ImportSummary EvidenceHint to ReadinessGap[] (one entry per missing evidence signal with affected diagnostics and manual next steps); I18-B: detectImportSource adds job.payload.model to hasModels check; PR #157 W-light: derives `sourceShape` as `hermes-cron` for nested usage token fields, `openclaw-export` for flat token/meta/job agent-turn shapes, otherwise `unknown`; exports `buildSourceShapeCopy(sourceShape)` for conservative read-only-first UI copy. |
...
| src/fixes.ts | 143 lines, buildFixCards + buildEvidenceBackedProblem + formatEvidenceBlurb; I14-B: buildEvidenceBackedProblem() returns evidence-backed problem text for CRITICAL ('Runs every N min, below the N min threshold.') and ERROR_WASTE ('N% error rate, above the N% threshold.') — strict typeof==='number' guard rejects null/''/whitespace before Number.isFinite(); all other categories fall back to FIX_LIBRARY exactly; I20: `buildFixCards` `order` array now includes `"LLM_AGENT_CRON"` between `"CRITICAL"` and `"ERROR_WASTE"` — was silently dropped before; I21: `buildFixCards` restructured — returns per-job cards (one job per card, not category-sliced), `issue !== 'OK'` filter excludes OK jobs, `lifecycleStatus` filter excludes disabled/historical, sorted by category rank then `totalTokens × errorRate` desc, no per-category slice limit |
...
| tests/parser.test.ts | 814 lines, parseJson / parseJsonl / parseZipEntries characterization tests (12 inline + 4 fixture-based) + detectImportSource / sourceShape tests + buildReadinessGaps tests + I11-A import readiness regression coverage; total 59 parser tests |
...
| tests/fixes.test.ts | 37 tests, buildEvidenceBackedProblem (15 tests) + buildFixCards with evidence-backed problem (15 tests) + I20 LLM_AGENT_CRON ordering (2 tests) + I21 active actionability (5 tests: OK filter, historical/disabled exclude, multi-job card split); coverage: CRITICAL/ERROR_WASTE with valid evidence, no evidence fallback, malformed-value fallback (null/""/whitespace), other categories no override, sort/slice preserved, action/impactLabel unchanged, job-level card split; total suite 502 tests |
| tests/i15a-openclaw-cli-guidance.test.ts | 149 tests, covers CLI syntax correctness (openclaw cron/* vs openclaw jobs/*), forbidden patterns (--resume/--no-agent-turn/--dry-run/cron expression strings), multi-ID command risk (whitespace-separated and comma-separated ID lists), local path references, ERROR_WASTE --id flag syntax, --enable flag, cron disable vs edit --disable; I21: ERROR_WASTE read-only first action structure tests (inspect-only, no edit/disable/enable first); whitespace-separated multi-ID asserts each command targets exactly one job ID; total test suite 502 tests |
...
| tests/advisory-signals.test.ts | 1201 lines, 116 tests — I23 + I24 + I25 + PR #157 W-light advisory review-signal contract tests: A1 zero-token fast failure; A2 premium model + repeated NO_REPLY; A3 cross-job shared failure signature; safety copy; ranking invariant; nested Hermes-style token-path coverage; A1 `runCount >= 2` noise-floor gate; strict `parseTokenNumber` whitelist tests; source-aware hermes-cron explanation amendments; default/openclaw-export/unknown explanation stability; threshold/first-action/ranking invariants with sourceShape; total suite 695 tests / 17 files |
| tests/source-shape-interpretation.test.ts | 150 lines, 16 tests — PR #157 W-light UI/copy contract tests for `buildSourceShapeCopy` and `renderImportSummary` source-shape interpretation section; verifies conservative read-only-first copy and forbidden user-facing phrases remain absent |
| tests/i22-agent-prompt-actionable.test.ts | ~400 lines, 48 tests — I22 agent diagnosis prompt primary action source-level UI contract tests: verifies Problem Summary / What To Do First / Agent Prompt visible flow (first action card default state); verifies old "Start here: inspect this job first" language absent from buildCardHtml; verifies first-card verification guidance collapsed under `<details class="advanced-details">`; verifies no `openclaw cron run` in first-card verification; verifies agent prompt classification output, prompt safety requirements, and generated guidance structure; total suite 554 tests / 15 files (at PR #139 merge — historical, current main @ 1b0592a is 695 tests / 17 files, see Current Local Validation Reality below) |
...
|----|-------|--------|-------------|
| #157 | feat(advisory): add source-aware Agent Cron Audit readiness framing (W-light) | 2026-06-05 | `1b0592a` |
| #156 | docs(PROJECT_STATE): refresh after PR #155 — I24 nested token schema + I25 strict parseTokenNumber | 2026-06-05 | `7a283dd` |
| #155 | fix(advisory): cover real Hermes nested token schema in A1; gate A1 on runCount>=2 | 2026-06-04 | `7c198c2` |
...
| #133 | fix(I21): make real smoke output actionable | 2026-05-13 | `6743403` |
| #131 | fix(I20): MVP closeout — hero copy, LLM_AGENT_CRON ordering, fix-card structure, stale-snapshot warning | 2026-05-12 | `8a32340` |
...
| I7B (Slice B — PRIVACY + SECURITY) | Issue #7 (I8) docs — Slice B | #112 | **SLICE COMPLETE** — Issue #7 CLOSED 2026-05-31 (RULES.md scope carried to Issue #5) |
...
| I24 (Hermes Nested Token Schema + A1 Noise-Floor Gate) | Real-schema follow-up — `pickPresentTokenValue` now reads Hermes-style nested `usage.total_tokens` / `usage.tokens` / `metrics.tokens` after top-level aliases; A1 requires `MIN_A1_RUN_COUNT = 2` qualifying runs per job (single-run patterns treated as dispatcher jitter); A3 `failureSignature` automatically benefits via shared helper; 21 new tests (de091d0 baseline); round 1 code review REQUEST CHANGES (nested token coercion trap) | #155 | **CLOSED** |
| I25 (Strict `parseTokenNumber` Token Whitelist) | I24 follow-up — nested `Number(inner)` path was still vulnerable to `Number('') / Number(false) / Number([]) → 0` false-zero trap (would let A1 fire and A3 group `zero-token-fast-failure` on records with no real numeric token count). New `parseTokenNumber(raw)` helper is a strict whitelist (finite numbers only; rejects null/undefined/blank/whitespace strings/booleans/objects/arrays/NaN/Infinity/all strings; numeric strings rejected in strict mode). `pickPresentTokenValue` rewritten so both top-level and nested paths delegate to `parseTokenNumber`. 39 new I25 tests (9 explicit trap-input cases + positive controls + adversarial harness) + A1 safety-copy test fixed (same jobId + `signals.length > 0` guard). 60 new tests total in this PR; suite 591 → 651 tests / 16 files; `src/advisory.ts` 653 → 773 lines; `tests/advisory-signals.test.ts` 525 → 973 lines. No src/main.ts / UI / docs / pricing / CLI / index.html / package.json change. No backend / network / telemetry / runtime-write / new dependency. Round 2 (post-amend) code review PASS | #155 | **CLOSED** |
| Agent Cron Audit source-aware readiness slice (W-light) | Minimal source-aware readiness slice. `src/advisory.ts`: optional `sourceShape` on A1/A2/A3 detectors, with hermes-cron explanation notes while thresholds, first actions, ranking, and default/openclaw-export/unknown copy remain stable. `src/parser.ts`: `detectImportSource` derives `sourceShape` (`hermes-cron` / `openclaw-export` / `unknown`) from in-memory data and exports `buildSourceShapeCopy`. `src/main.ts`: renders "Source-shape interpretation" in import summary when present; still does not import `src/advisory.ts`, so advisory remains dormant/opt-in. Tests: advisory 116/116, parser 59/59, source-shape interpretation 16/16, full suite 695/695, no-network 2/2, build clean at 60.44 kB / 18.71 kB gzip. Changed files limited to `src/advisory.ts`, `src/main.ts`, `src/parser.ts`, `tests/advisory-signals.test.ts`, `tests/parser.test.ts`, `tests/source-shape-interpretation.test.ts`; no docs/index/package/lockfile/backend/network/telemetry/runtime-write/new dependency/ClawHub skill workflow. guardian_cat R2 PASS comments: `4624904268`, `4624925737`. | #157 | **CLOSED** |
...
| I20 (MVP Closeout Sprint) | index.html: hero subhead copy tightened to "TokenSave audits recurring AI agent jobs for avoidable token burn. Drop your OpenClaw export — Import → Diagnose → Evidence → Manual Fix. All analysis stays local."; section title "Top Waste Jobs" renamed to "Active Recurring Waste Findings" with updated subtitle; src/fixes.ts: `buildFixCards` `order` array now includes `"LLM_AGENT_CRON"` between `"CRITICAL"` and `"ERROR_WASTE"` — was silently dropped before (no fix cards rendered for that category); src/main.ts: fix card HTML restructured with explicit Problem/Evidence/Manual Fix/Verify section labels; stale-snapshot amber warning in renderImportSummary — only fires when `lifecycleStatus === 'historical'` synthetic/unmatched run-only records present (disabled jobs alone do not trigger); OK fix cards suppress misleading "Token waste patterns detected" evidence fallback text; ERROR_WASTE verify section includes inline stale-snapshot re-export note; tests/fixes.test.ts: 3 new tests for LLM_AGENT_CRON ordering (existence, relative position before ERROR_WASTE, after CRITICAL); tests/i17a: updated subtitle test; validation: npm run build pass, npm test 500/500 pass / 13 files, no-network test 2/2 pass | #131 | **CLOSED** |
| I21 (Real Smoke Actionability Fix) | src/fixes.ts: `buildFixCards` returns job-level cards (one job per card) instead of category-sliced; filters out OK jobs (`issue !== 'OK'`), disabled jobs (`lifecycleStatus === 'disabled'`), historical jobs (`lifecycleStatus === 'historical'`) from active action cards; no per-category slice limit; sorted by category rank then `totalTokens × errorRate` desc. src/main.ts: renderFixes active card loop outputs per-job cards, each with "Start here: inspect this job first" header on card 0 (`index === 0`) + `.first-action-card` cyan CSS highlight; "Why This Is First" / "Why Inspect This Job" section using `buildWhyFirstText()` per category; "Safe Inspect Command" section using `buildInspectSteps()` for ERROR_WASTE and `buildInspectSteps()` for all other categories (read-only `cron show` + `cron runs --id --limit 5`); "Manual Diagnosis Guidance" inline for ERROR_WASTE; historical/disabled review moved to collapsible `<details class="historical-review">` with `<summary>` (not auto-expanded); stale-snapshot warning scoped to `hasSyntheticJobs` only, copy clarifies "Active jobs with current job definitions are not marked stale by this warning."; `historicalFixes` simplified to `historicalJobs.filter(job => job.badge !== 'OK')` (OK historical jobs invisible even in collapsed section); OK historical jobs now fully suppressed. index.html: `.first-action-card` CSS (grid-column, cyan border/gradient), `.start-here` label style, `.historical-review` / `.historical-review-list` / `summary` / `.historical-card` styles, `restraint-card` / `.restraint-message` styles, Cost by Job pie title → "Approx. Cost by Job (Secondary)". tests/i21-real-smoke-actionability.test.ts: 7 source-scanning contract tests (ERROR_WASTE read-only inspect, non-ERROR_WASTE inspect-only, start-here header, historical re-export guidance, OK restraint, stale warning scope, Cost demotion). tests/fixes.test.ts: 5 new I21 tests (OK filter, historical/disabled exclude, multi-job card split). tests/i15a: 4 new I21 tests (ERROR_WASTE read-only first action structure). validation: npm run build pass, npm test 502/502 pass / 14 files, no-network test 2/2 pass | #133 | **CLOSED** |
...
**I23 (Advisory Review-Signal Layer, PR #152) — CLOSED.** Field-test follow-up. Strict D1-D7 rule layer produced only 1 confirmed finding from 23 jobs / 72 errors, while cross-job signature analysis produced 3 actionable signals that did not match any D-rule threshold. New `src/advisory.ts` adds a strictly lower-priority A1/A2/A3 review-signal layer: A1 zero-token fast failure, A2 premium model + repeated NO_REPLY, A3 cross-job shared failure signature. Every signal includes the literal "Review signal (not confirmed waste)" disclaimer; A1 emits `{lowUsd: 0, highUsd: 1}` only because zero-token runs that did not invoke the model have a known narrow bound; A2 and A3 emit `approximateCostExposure: null` (no inferred cost). `pickPresentTokenValue` helper uses `Object.prototype.hasOwnProperty.call` to avoid the `Number(null) === 0` false-positive on missing token fields. First action is always a read-only `openclaw cron runs --id <id> --limit 5` or `cat ~/.openclaw/cron/jobs.json | head -100`; `disable` / `edit` / `restart` / `switch-model` are forbidden in advisory copy. Confirmed D-rule findings remain primary via `rankAdvisorySignals` demotion. 37 new tests; total suite 591 / 16 files. NOT imported by `src/main.ts` (opt-in). Round 4 code review PASS.

**I24 (Hermes Nested Token Schema + A1 Noise-Floor Gate, PR #155) — CLOSED.** Real-schema follow-up. Dataset E (6 real Hermes cron jobs, 3,206 redacted runs) showed A1 firing 0 times because real Hermes cron run records keep the token count under a nested `usage.total_tokens` object, not at the top level. `pickPresentTokenValue` extended to read three nested paths (`usage.total_tokens` / `usage.tokens` / `metrics.tokens`) after the existing top-level aliases, each gated on outer object being a real own-property object (not null, not a primitive) and inner value being finite. New `MIN_A1_RUN_COUNT = 2` constant — jobs with fewer than 2 qualifying zero-token fast-failure runs are skipped before any signal is constructed (single-run patterns treated as dispatcher jitter). A1 evidence `sourceFields` updated to include the new nested paths. A2 and A3 unchanged; A3 `failureSignature` automatically benefits via shared helper. 21 new tests at de091d0 baseline. Round 1 code review REQUEST CHANGES (nested `Number(inner)` still vulnerable to `Number('') / Number(false) → 0` coercion trap).

**I25 (Strict `parseTokenNumber` Token Whitelist, PR #155) — CLOSED.** I24 follow-up. The nested `Number(inner)` path remained vulnerable to `Number('') / Number(false) / Number([]) → 0` coercion — with two fast-failed records this would satisfy A1's zero-token / `runCount >= 2` gate and A3's `zero-token-fast-failure` grouping, even though no real numeric token count was actually present. New `parseTokenNumber(raw)` helper is a strict whitelist: accepts finite numbers only; rejects `null` / `undefined` / blank and whitespace strings / booleans / objects / arrays / `NaN` / `Infinity` / all strings (numeric strings rejected in strict mode — explicit conservative choice). `pickPresentTokenValue` rewritten so both top-level and nested paths delegate to `parseTokenNumber`, closing the trap at every path. A1 safety-copy test tightened: both records now share the same `jobId` AND the test asserts `signals.length > 0` before scanning forbidden words (it no longer passes vacuously on the `runCount >= 2` gate). 39 new I25 tests (9 explicit trap-input cases + positive controls + adversarial harness) + A1 safety-copy fix. 60 new tests total in #155; suite 591 → 651 tests / 16 files; `src/advisory.ts` 653 → 773 lines; `tests/advisory-signals.test.ts` 525 → 973 lines. No `src/main.ts` / UI / docs / pricing / CLI / `index.html` / `package.json` change. No backend / network / telemetry / runtime-write / new dependency. Round 2 (post-amend) code review PASS. NOT imported by `src/main.ts` (still opt-in, module remains dormant until a follow-up wire-up PR).

**Agent Cron Audit source-aware readiness slice (W-light, PR #157) — CLOSED.** Minimal visible readiness slice after BG selected W-light. `src/advisory.ts` adds optional sourceShape-aware explanation amendments for A1/A2/A3 only when called with `hermes-cron`; openclaw-export/unknown/default explanation text remains byte-for-byte stable, and thresholds / first actions / ranking remain unchanged. `src/parser.ts` derives sourceShape from in-memory dataset shape (`hermes-cron` for nested usage token fields, `openclaw-export` for flat token/meta/job agent-turn shapes, `unknown` otherwise) and exports `buildSourceShapeCopy`. `src/main.ts` renders a Source-shape interpretation paragraph in import summary when present; it still does not import `src/advisory.ts`, so advisory remains dormant/opt-in. 44 tests added: advisory 97 → 116, parser 50 → 59, new source-shape interpretation 16. Final validation: 695/695 tests across 17 files, no-network 2/2, build clean at 60.44 kB / 18.71 kB gzip. Merge details: old main `7a283ddef8595f8a9ffc3c319e35ecf8d9658429`, merge commit `1b0592a19bf663a40745dcb0992e16daaf292609`.

**Recommended Next Step**: Stop after this docs-only PROJECT_STATE refresh PR receives guardian_cat review. Do not start the next implementation PR without BG direction. The next implementation decision remains separate: whether and how to wire the advisory layer into `analyzeDataset` / `renderReport` now that source-shape interpretation is visible but advisory remains dormant.
...
**I17-A (Priority Basis Line) — CLOSED (PR #123).** Active top waste cards now show "Priority basis" metadata line explaining waste estimation method. Focus remains on real-data audit usefulness / Import → Diagnose → Evidence → Manual Fix.

**I16 (UX-only .rar Import Guidance) — CLOSED (PR #121).** Fail-fast RAR rejection before `file.text()` load in `ingestLooseFile()`, `mapErrorMessage()` RAR branch mapped to local-extraction guidance. No RAR parser, no WASM, no backend, no telemetry.
...
- Do not start pricing, pre-flight rules, extension, export/report/dashboard, or parser expansion without BG approval
- No new implementation slices until BG approves next planning PR.

## Current Local Validation Reality

- `npm run build` passes (PR #157 final validation, 2026-06-05T08:42:57Z): `dist/index.html` 13.15 kB, `dist/app/index.html` 35.30 kB, `dist/assets/app-*.js` 60.44 kB (gzip 18.71 kB). The +1.56 kB / +0.55 kB gzip delta vs PR #155 baseline is from sourceShape detection + `buildSourceShapeCopy`, and remains within the W-light budget.
- `npm test` / `npx vitest run` passes 695 tests (17 test files), including 116 in `tests/advisory-signals.test.ts`, 59 in `tests/parser.test.ts`, and 16 in `tests/source-shape-interpretation.test.ts`.
- `tests/no-network.test.ts` passes 2/2 and handles Vite modulepreload polyfill false positives from generated `dist/assets` output.
- Source/runtime scan catches forbidden app network APIs: fetch, XMLHttpRequest, sendBeacon, WebSocket, EventSource.
- Forbidden surfaces for PR #157 were absent: no `docs/*` in implementation diff, no `index.html`, no package/lockfile/config changes, no `dist/` / `node_modules/`, no backend/network/telemetry/runtime write path, no new dependencies, and no ClawHub skill workflow.
...
| #7 | I8: README, MVP_SPEC, PRIVACY, SECURITY, RULES docs | **CLOSED (COMPLETED)** | README.md (PR #110), MVP_SPEC.md (PR #110), PRIVACY.md (PR #112), SECURITY.md (PR #112) merged. RULES.md scope carried to Issue #5 (B1-B3/W1-W5) — Issue #7 CLOSED 2026-05-31, completion reason COMPLETED. |
