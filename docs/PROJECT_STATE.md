...
**Last updated**: 2026-05-12T13:50:00Z
**Source**: GitHub `origin/main` at commit `8a32340` (PR #131 fix(I20): MVP closeout — hero copy, LLM_AGENT_CRON ordering, fix-card structure, stale-snapshot warning; merge commit `8a32340`)
...
| Main branch SHA | `8a32340` (PR #131 fix(I20): MVP closeout — hero copy, LLM_AGENT_CRON ordering, fix-card structure, stale-snapshot warning; merge commit `8a32340`) |
| Package manager | npm |
...
| I20 (MVP Closeout Sprint) | index.html: hero subhead copy tightened to "TokenSave audits recurring AI agent jobs for avoidable token burn. Drop your OpenClaw export — Import → Diagnose → Evidence → Manual Fix. All analysis stays local."; section title "Top Waste Jobs" renamed to "Active Recurring Waste Findings" with updated subtitle; src/fixes.ts: `buildFixCards` `order` array now includes `"LLM_AGENT_CRON"` between `"CRITICAL"` and `"ERROR_WASTE"` — was silently dropped before (no fix cards rendered for that category); src/main.ts: fix card HTML restructured with explicit uppercase Problem/Evidence/Manual Fix/Verify section labels; stale-snapshot amber warning in renderImportSummary — only fires when `lifecycleStatus === 'historical'` synthetic/unmatched run-only records present (disabled jobs alone do not trigger); OK fix cards suppress misleading "Token waste patterns detected" evidence fallback text (empty string); ERROR_WASTE verify section includes inline stale-snapshot re-export note; tests/fixes.test.ts: 3 new tests for LLM_AGENT_CRON ordering (existence, relative position before ERROR_WASTE, after CRITICAL); tests/i17a: updated subtitle test from "Ranked by recurring waste priority" to "Ranked by recurring token waste"; validation: npm run build pass (25.98 kB HTML / 51.15 kB JS), npm test 500/500 pass / 13 files, no-network test 2/2 pass | #131 | **CLOSED** |
...
- `npm test` passes 500 tests (PR #131: 3 new tests for LLM_AGENT_CRON ordering + 1 test update for section rename; total suite 500 tests / 13 files).
